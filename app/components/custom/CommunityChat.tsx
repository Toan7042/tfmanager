  ...
  export default function CommunityChat() {
    ...
    const handlePinMessage = async (messageId: number) => {
      ...
      const res = await fetch(`/api/community-chatapp/messages/update`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) console.error("Failed to pin message:", await res.text());
    };
    ...
    const handleDeleteMessage = async (messageId: number) => {
      ...
      if (!res.ok) console.error("Failed to delete message:", await res.text());
    };
            ...
            <div className="p-1 bg-gray-50 border-b pinned-messages">
              {pinnedMessages.map((msg) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="flex items-center gap-1 text-sm text-blue-500 cursor-pointer p-1 rounded group hover:bg-blue-100 transition-colors duration-200"
