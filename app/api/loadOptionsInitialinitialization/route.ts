...
export async function GET() {
  try {
    ...
    const countryData = ["NONE", ...fs.readFileSync(path.join(basePath, "file_option_country.txt"), "utf-8").split("\n").map(line => line.trim()).filter(Boolean)];
    ...
    // LOAD JSON
    const phoneModels = ["NONE", "RAND-ITEM", ...JSON.parse(fs.readFileSync(path.join(basePath, "jsonPhoneInfo_data.json"), "utf-8")).devices.map((device: { MODEL: string }) => device.MODEL)];
    const carrierData = ["NONE", "AS-COUNTRY", ...JSON.parse(fs.readFileSync(path.join(basePath, "jsonCarrier_data.json"), "utf-8")).map((device: { ISO: string }) => device.ISO)];    
