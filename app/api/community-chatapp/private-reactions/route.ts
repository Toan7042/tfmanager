...
export async function POST(request: Request) {
  ...
  const existingReaction = await prisma.privateReaction.findFirst({
    where: { messageId, userId: Number(session.user.id), emoji },
  });

  let reaction;
  if (!existingReaction) {
    reaction = await prisma.privateReaction.create({
      data: { messageId, userId: Number(session.user.id), emoji },
    });
  }
  ...
  if (message) {
    ...
    await pusher.trigger(`private-chat-${message.receiverId}`, "new-private-reaction", formattedMessage);
  }
